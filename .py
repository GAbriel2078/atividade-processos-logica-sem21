...
def registrar_transacao(self, tipo, valor):
    self.__transacoes.append({"Tipo": tipo, "Valor": valor})
...
def emitir_cheque(self, valor):
    if self.__saldo + self.limite_cheque_especial >= valor:
       self.__saldo -= valor
       self.__registrar_transacao("Emissão de Cheque", valor)
    else:
...
class ContaPoupanca(ContaBancaria):
    def calcular_juros_mensal(self, taxa_juros):
        ...
        self.__saldo += juros
        self.__registrar_transacao("Juros Mensais", juros)
class ContaInvestimento(ContaBancaria):
    def __init__(self, numero_conta):
        super().__init__(numero_conta)
        self.__investimentos = []
def realizar_investimento(self, produto, valor):
    self.__registrar_transacao("Investimento", valor)
